...
#if 0 // WRONG ping pong
static uint8_t  rx_buffer[2][BT_BUFFER_SIZE] __attribute__ ((aligned(BT_BUFFER_ALIGN))); // Buffer to receive data
static uint8_t* rx_dest;       // Pointer to receive data
static uint32_t rx_len = 0;        // Total bytes to receive

static uart_dma_rx_init() {
    const unsigned int chNum = EDMA3_CHA_UART2_RX, tccNum = EDMA3_CHA_UART2_RX;

    EDMA3CCPaRAMEntry paramSet, paramSetPong;

    /* Fill the PaRAM Set with transfer specific information */
    paramSet.srcAddr = p_uart + UART_RBR;
    paramSet.destAddr = (unsigned int) rx_buffer[0];
    paramSet.aCnt = 1/*MAX_ACNT*/;
    paramSet.bCnt = BT_BUFFER_SIZE;
    paramSet.cCnt = 1/*MAX_CCNT*/;
    ...
    paramSet.destCIdx = 0;
    paramSet.linkAddr = EDMA3CC_OPT(EDMA3_CHA_UART2_RX_PONG); //(unsigned short)0xFFFFu;
    paramSet.bCntReload = BT_BUFFER_SIZE;
    paramSet.opt = 0x00000000u;
    paramSet.opt |= ((EDMA3CC_OPT_SAM) << EDMA3CC_OPT_SAM_SHIFT);
    paramSet.opt |= ((tccNum << EDMA3CC_OPT_TCC_SHIFT) & EDMA3CC_OPT_TCC);
//    paramSet.opt |= (1 << EDMA3CC_OPT_TCINTEN_SHIFT);
...
//  EDMA3DisableTransfer(SOC_EDMA30CC_0_REGS, chNum, EDMA3_TRIG_MODE_EVENT);
    ...
    paramSetPong = paramSet;
    paramSetPong.destAddr = (unsigned int) rx_buffer[1];
    paramSetPong.linkAddr = EDMA3CC_OPT(EDMA3_CHA_UART2_RX);

    /* Now write the PaRAM Set */
    EDMA3SetPaRAM(SOC_EDMA30CC_0_REGS, chNum, &paramSet);
    EDMA3SetPaRAM(SOC_EDMA30CC_0_REGS, EDMA3_CHA_UART2_RX_PONG, &paramSetPong);
...
static uint32_t rx_head = 0, rx_tail = 0; // logical head/tail pointers of rx buffer
void bt_dma_cyc(intptr_t exinf) {
    ...
    EDMA3CCPaRAMEntry paramSet;
    EDMA3GetPaRAM(SOC_EDMA30CC_0_REGS, EDMA3_CHA_UART2_RX, &paramSet);
    if (paramSet.bCnt != BT_BUFFER_SIZE) { // tail is in ping buffer
        new_tail = BT_BUFFER_SIZE - paramSet.bCnt;
    } else {
        EDMA3GetPaRAM(SOC_EDMA30CC_0_REGS, EDMA3_CHA_UART2_RX_PONG, &paramSet);
        if (paramSet.bCnt != BT_BUFFER_SIZE || (rx_tail > 0 && rx_tail <= BT_BUFFER_SIZE)) { // tail is in pong buffer OR (ping buffer is FULL and pong buffer is empty)
            new_tail = BT_BUFFER_SIZE + BT_BUFFER_SIZE - paramSet.bCnt;
        } else new_tail = 0; // rx_tail == 0 || rx_tail >= BT_BUFFER_SIZE
        // rx_tail = 0 -> new = 0, 0 < rx_tail < BT_BUFFER_SIZE -> new = BT_BUFFER_SIZE, rx_tail >= BT_BUFFER_SIZE -> new = 0
    }
    uint32_t rx_buf_len = (new_tail < rx_head) ? (new_tail + BT_BUFFER_SIZE * 2 - rx_head) : (new_tail - rx_head);
    if (rx_tail != new_tail) syslog(LOG_ERROR, "[bluetooth] new RX buffer tail: %d, len: %d", new_tail, rx_buf_len);
    rx_tail = new_tail;
    ...
    if (rx_buf_len >= rx_len && rx_len != 0) {
        ...
        int   rx_len_in_buf1 = 0, rx_len_in_buf2 = 0;
//        new_tail = (rx_tail < rx_head) ? (rx_tail + BT_BUFFER_SIZE * 2) : rx_tail;
        if (rx_head < BT_BUFFER_SIZE) {                     // start from buf1
            rx_ptr_in_buf1 = &(rx_buffer[0][rx_head]);
            rx_len_in_buf1 = BT_BUFFER_SIZE - rx_head;
            if (rx_len_in_buf1 < rx_len) {                  // buf2 also holds RX data
                rx_ptr_in_buf2 = rx_buffer[1];
                rx_len_in_buf2 = rx_len - rx_len_in_buf1;
            } else rx_len_in_buf1 = rx_len;
        } else {                                            // start from buf2
            rx_ptr_in_buf2 = &(rx_buffer[1][rx_head - BT_BUFFER_SIZE]); // (void*rxbuffer)+rx_head
            rx_len_in_buf2 = BT_BUFFER_SIZE * 2 - rx_head;
            if (rx_len_in_buf2 < rx_len) {                  // buf1 also holds RX data
                rx_ptr_in_buf1 = rx_buffer[0];
                rx_len_in_buf1 = rx_len - rx_len_in_buf2;
            } else rx_len_in_buf2 = rx_len;
        }

        CacheDataInvalidateBuff(rx_ptr_in_buf1, rx_len_in_buf1);
        CacheDataInvalidateBuff(rx_ptr_in_buf2, rx_len_in_buf2);

        if (rx_head < BT_BUFFER_SIZE) {
            memcpy(rx_dest, rx_ptr_in_buf1, rx_len_in_buf1);
            memcpy(rx_dest + rx_len_in_buf1, rx_ptr_in_buf2, rx_len_in_buf2);
        } else {
            memcpy(rx_dest, rx_ptr_in_buf2, rx_len_in_buf2);
            memcpy(rx_dest + rx_len_in_buf2, rx_ptr_in_buf1, rx_len_in_buf1);
        }

//        CacheDataInvalidateBuff(rx_buffer, rx_len); // TODO: should we invalidate it earlier? or disable cache
//        memcpy(rx_dest, rx_buffer, rx_len);
        rx_head += rx_len;
        if (rx_head >= BT_BUFFER_SIZE * 2) rx_head -= BT_BUFFER_SIZE * 2;
        rx_len = 0;
        rx_cb();
    }
...
#endif
        if (rx_len == 0) {
            syslog(LOG_ERROR, "[bluetooth] dummy RX dma event");
            return;
        }
        assert(rx_len != 0);
