 ...
 *  の責任を負わない．
 * 
 *  $Id: mempfix.c 1001 2014-04-20 18:13:30Z ertl-hiro $
 */
...
acre_mpf(const T_CMPF *pk_cmpf)
{
	...
	CHECK_RSATR(pk_cmpf->mpfatr, TA_TPRI|TA_DOMMASK);
	domid = get_atrdomid(pk_cmpf->mpfatr);
	CHECK_ATRDOMID_INACTIVE(domid);
	CHECK_PAR(pk_cmpf->blkcnt != 0);
	CHECK_PAR(pk_cmpf->blksz != 0);
	CHECK_NOSPT(pk_cmpf->mpf != NULL);
	if (pk_cmpf->mpf != NULL) {
		CHECK_ALIGN_MPF(pk_cmpf->mpf);
		CHECK_OBJ(valid_memobj(pk_cmpf->mpf,
						ROUND_MPF_T(pk_cmpf->blksz) * pk_cmpf->blkcnt));
	}
	if (pk_cmpf->mpfmb != NULL) {
		CHECK_ALIGN_MB(pk_cmpf->mpfmb);
		CHECK_OBJ(valid_memobj_kernel(pk_cmpf->mpfmb,
									sizeof(MPFMB) * pk_cmpf->blkcnt));
	}
	...
	if (tnum_mpf == 0 || queue_empty(&free_mpfcb)) {
		...
		if (p_mpfmb == NULL) {
			...
			p_mpfinib->acvct.acptn3 = acptn | rundom;
