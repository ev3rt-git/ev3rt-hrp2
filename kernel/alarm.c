...
acre_alm(const T_CALM *pk_calm)
{
	...
	CHECK_RSATR(pk_calm->almatr, TA_NULL|TA_DOMMASK);
	CHECK_ATRDOMID_KERNEL(get_atrdomid(pk_calm->almatr));
	...
	if (tnum_alm == 0 || queue_empty(&free_almcb)) {
		...
		p_alminib->acvct.acptn1 = TACP_KERNEL;
		p_alminib->acvct.acptn2 = TACP_KERNEL;
		p_alminib->acvct.acptn3 = TACP_KERNEL | rundom;
		p_alminib->acvct.acptn4 = TACP_KERNEL;
