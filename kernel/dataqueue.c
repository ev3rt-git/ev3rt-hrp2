 ...
 *  の責任を負わない．
 * 
 *  $Id: dataqueue.c 994 2014-04-18 08:48:40Z ertl-hiro $
 */
...
acre_dtq(const T_CDTQ *pk_cdtq)
{
	...
	CHECK_RSATR(pk_cdtq->dtqatr, TA_TPRI|TA_DOMMASK);
	domid = get_atrdomid(pk_cdtq->dtqatr);
	CHECK_ATRDOMID_INACTIVE(domid);
	if (pk_cdtq->dtqmb != NULL) {
		CHECK_ALIGN_MB(pk_cdtq->dtqmb);
		CHECK_OBJ(valid_memobj_kernel(pk_cdtq->dtqmb,
									sizeof(DTQMB) * pk_cdtq->dtqcnt));
	}
	...
	if (tnum_dtq == 0 || queue_empty(&free_dtqcb)) {
		...
		if (pk_cdtq->dtqcnt != 0 && p_dtqmb == NULL) {
			...
			p_dtqinib->acvct.acptn3 = acptn | rundom;
