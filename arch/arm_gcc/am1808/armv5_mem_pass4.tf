...
$ Nonglobalページに関する定義の生成

$ Nonglobalページ領域の使用状況の表示
$ ページテーブル領域の確保数の計算
$IF LENGTH(ARM_NONGLOBAL_PAGES_NUM)$
	$nonglobal_entries_size = ARM_NONGLOBAL_PAGES_NUM$
$ELSE$
	$nonglobal_entries_size = 10$
$END$

$WARNING$
	$FORMAT(_("%d/%d of the allocated nonglobal page entries area is used"),
								LENGTH(nonglobal_entries), nonglobal_entries_size)$
$END$

$ ページテーブル領域が足りない場合のエラー処理
$IF LENGTH(nonglobal_entries) > nonglobal_entries_size$
	$ERROR$
...
$NL$
#define ARM_NONGLOBAL_PAGES_NUM	$nonglobal_entries_size$$NL$
$NL$
...
$ELSE$
const uint32_t _kernel_nonglobal_pages[ARM_NONGLOBAL_PAGES_NUM] = {0};$NL$
$END$
