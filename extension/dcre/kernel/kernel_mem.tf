...
$
$   Copyright (C) 2011-2013 by Embedded and Real-Time Systems Laboratory
...
$   の責任を負わない．
$
$   $Id: kernel_mem.tf 994 2014-04-18 08:48:40Z ertl-hiro $
$  
$ =====================================================================
...
$		// メモリオブジェクトの領域に重なりがある場合
		$IF OMIT_CHECK_USTACK_OVERLAP
					&& MO.TYPE[moid] == TOPPERS_USTACK && !MO.LINKER[moid]
...
/*$NL$
$SPC$*  Memory Object Initialization Information$NL$
$SPC$*/$NL$
$NL$
...
$	// memtop_tableで参照するラベルの宣言の生成
	$FOREACH moid MO_MEMTOP_ORDER$
		$IF MO.LINKER[moid]$
			...
			$IF !LENGTH(MO.SUPPRESSLIMIT[moid])$
				extern char __limit_$MO.MLABEL[ALT(MO.MERGELAST[moid],moid)]$;
				$NL$
			$END$
		$END$
	$END$$NL$
...
$	// サイズが0でないdataセクションのリスト（MO_DATASEC_LIST）を
...
$IF !OMIT_STANDARD_DATASECINIB$
	/*$NL$
	$SPC$*  Data Section Initialization Information$NL$
	$SPC$*/$NL$
	$NL$
...
$	// サイズが0でないbssセクションのリスト（MO_BSSSEC_LIST）を
$	// パス3で作成していない場合には，ここで作成する．
	$MO_BSSSEC_LIST = {}$
...
$IF !OMIT_STANDARD_BSSSECINIB$
	/*$NL$
	$SPC$*  BSS Section Initialization Information$NL$
	$SPC$*/$NL$
	$NL$
...
$		// メモリ保護単位の先頭番地と上限番地を取り出す
		$IF MO.LINKER[moid]$
			$IF EQ(MO.PLABEL[moid], "")$
				$baseaddr = START_SYMBOL(MO.SLABEL[moid])$
				$limitaddr = LIMIT_SYMBOL(MO.SLABEL[moid])$
			$ELSE$
				$baseaddr = START_SYMBOL(MO.PLABEL[moid])$
				$limitaddr = LIMIT_SYMBOL(MO.PLABEL[moid])$
			$END$
		$ELSE$
...
$FUNCTION CHECK_MEMOBJ_KERNEL$
	...
	$IF moid && (MO.MEMATR[moid] & (TA_NOWRITE|TA_NOREAD)) == 0
...
$	// 拡張サービスコールの先頭番地のチェック
	...
	$FOREACH order SVC.ORDER_LIST$
		$svcrtn = PEEK(svcinib + sizeof_SVCINIB * (SVC.FNCD[order] - 1)
										+ offsetof_SVCINIB_svcrtn, sizeof_FP)$
		$IF CHECK_FUNC_ALIGN && (svcrtn & (CHECK_FUNC_ALIGN - 1)) != 0$
			$ERROR SVC.TEXT_LINE[order]$E_PAR: 
				$FORMAT(_("%1% `%2%\' of `%3%\' in %4% is not aligned"),
				"svcrtn", SVC.SVCRTN[order], SVC.FNCD[order], "DEF_SVC")$$END$
		$END$
		$IF CHECK_FUNC_NONNULL && svcrtn == 0$
			$ERROR SVC.TEXT_LINE[order]$E_PAR: 
				$FORMAT(_("%1% `%2%\' of `%3%\' in %4% is null"),
				"svcrtn", SVC.SVCRTN[order], SVC.FNCD[order], "DEF_SVC")$$END$
		$END$
