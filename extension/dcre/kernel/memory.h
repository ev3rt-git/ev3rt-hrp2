 ...
 *  の責任を負わない．
 * 
 *  $Id: memory.h 994 2014-04-18 08:48:40Z ertl-hiro $
 */
...
/*
 *  メモリオブジェクトに含まれているかのチェック
 *
 *  指定したメモリ領域がメモリオブジェクトに含まれているかどうかをチェッ
 *  クし，含まれている場合にtrue，そうでない場合にfalseを返す．
 */
extern bool_t	valid_memobj(const void *base, SIZE size);

/*
 *  カーネル専用のメモリオブジェクトに含まれているかのチェック
 *
 *  指定したメモリ領域が，カーネル専用のメモリオブジェクトに含まれてい
 *  るかどうかをチェックし，含まれている場合にtrue，そうでない場合に
 *  falseを返す．
 */
extern bool_t	valid_memobj_kernel(const void *base, SIZE size);
