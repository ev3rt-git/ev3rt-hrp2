 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: memory.c 994 2014-04-18 08:48:40Z ertl-hiro $
 */
 ...
 */
#ifdef TOPPERS_memvalm

bool_t
valid_memobj(const void *base, SIZE size)
{
	int_t	meminib;
	ATR		mematr;
	SIZE	memsize;
...
#endif /* TOPPERS_memvalm */

/*
 *  カーネル専用のメモリオブジェクトに含まれているかのチェック
 */
#ifdef TOPPERS_memvalk

bool_t
valid_memobj_kernel(const void *base, SIZE size)
{
	int_t	meminib;
	ATR		mematr;
	SIZE	memsize;
	...
	else if ((mematr & (TA_NOWRITE|TA_NOREAD)) != 0U
